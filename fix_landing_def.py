...
# Write a clean Landing.jsx using raw string to avoid any escaping issues
...
open_nav = c.count('<nav')
close_nav = c.count('</nav>')
open_footer = c.count('<footer')
close_footer = c.count('</footer>')

print(f"<div>: {open_div}, </div>: {close_div}, balanced: {open_div == close_div}")
print(f"<section>: {open_section}, </section>: {close_section}, balanced: {open_section == close_section}")
print(f"<nav>: {open_nav}, </nav>: {close_nav}, balanced: {open_nav == close_nav}")
print(f"<footer>: {open_footer}, </footer>: {close_footer}, balanced: {open_footer == close_footer}")
print(f"Total lines: {c.count(chr(10))}")
print("ALL TAGS BALANCED!" if (
    open_div == close_div and
    open_section == close_section and
    open_nav == close_nav and
    open_footer == close_footer
) else "TAGS NOT BALANCED!")
