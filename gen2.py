...
# Build the JSX by chunks to ensure perfect tag matching
parts = []
...
const features = [
  { icon: MessageCircle, emoji: "💬", title: "AI Health Chat",
    desc: "Talk to CareAI, powered by GPT-4o-mini. Get warm health guidance anytime.",
    color: "bg-purple-100 text-purple-700" },
  { icon: Stethoscope, emoji: "🩺", title: "Symptom Checker",
...
parts.append("""        <div className="lg:col-span-5">
...
parts.append("""      <section className="relative z-10 max-w-6xl mx-auto px-5 lg:px-8 pb-20">
...
parts.append("""      <footer className="relative z-10 max-w-7xl mx-auto px-5 lg:px-8 pb-8">
...
# Validate
open_div = full.count('<div')
close_div = full.count('</div>')
open_sec = full.count('<section')
close_sec = full.count('</section>')
print(f"Line count: {full.count(chr(10))}")
print(f"<div: {open_div}, </div>: {close_div}, balanced: {open_div == close_div}")
